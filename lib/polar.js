...
// Simpele lap-extractie uit TCX-XML zonder externe dependency (polar.js draait
// server-side; we willen hier geen extra parser-lib-afhankelijkheid voor een
// paar regels regex). Elke <Lap>-tag wordt één ronde/kilometer-split.
function extractLapsFromTcx(xmlText) {
  const laps = [];
  const lapMatches = xmlText.match(/<Lap[^>]*>[\s\S]*?<\/Lap>/g) || [];
  for (const lapXml of lapMatches) {
    const num = (re) => {
      const m = lapXml.match(re);
...
// nog-niet-gecommitte transactie gebeuren) en geeft de losse rondes terug.
export async function fetchExerciseLaps(accessToken, exerciseUri) {
  try {
    ...
    return extractLapsFromTcx(text);
  } catch {
    return [];
  }
}
...
export async function fetchNewPolarExercises(accessToken, polarUserId) {
  ...
  for (const exerciseUri of exercises) {
    ...
    if (exRes.ok) {
      ...
      const laps = await fetchExerciseLaps(accessToken, exerciseUri);
      const samplesProbe = await fetchExerciseSampleProbe(accessToken, exerciseUri);
      results.push({ ...exercise, _laps: laps, _samples_probe: samplesProbe });
    }
  }
...
export function normalizePolarExercise(ex) {
  ...
  return {
    ...
    laps: ex._laps && ex._laps.length > 0 ? ex._laps : null,
    raw: ex,
  };
}
